...
async function run(){
  try{
    ...
    periods[key2] = p2;
    await pool.close();
    const out = { sbu:'FAL', sbuName:'Fariq Agro Ltd.', source:'DataMart.inv.tblInventoryStatement + itm.numAverageRate (iBOS)', window:'6-mo trailing from 1st (as of '+asOf1+')', periods };
    fs.writeFileSync('C:\\Users\\saada\\OneDrive\\Documents\\Default Project\\slob_ibos_fal.js','const SLOB_IBOS_FAL = '+JSON.stringify(out)+';');
    console.log('Saved slob_ibos_fal.js (iBOS) as-of '+asOf1);
