...
// Warehouse filter: 4 plants as requested — exact + LIKE fallback for Tea variant
...
async function period(pool, asOf, key, codeMap){
  ...
  // closing value = ClosingStock * AvgRate (statement rate) — per item/warehouse latest
  const q1 = `WITH b AS (SELECT *, ROW_NUMBER() OVER (PARTITION BY [Business Unit], strItemName, Warehouse ORDER BY dteTransactionDate DESC) AS rn ${base}), latest AS (SELECT * FROM b WHERE rn=1) SELECT l.strItemName, SUM(l.ClosingStock * l.AvgRate) AS closingVal, MAX(ISNULL(itm.strItemTypeName,'Other')) AS typeName FROM latest l LEFT JOIN (SELECT strItemName, strItemTypeName, ROW_NUMBER() OVER (PARTITION BY strItemName ORDER BY dteLastActionDateTime DESC) rn FROM itm.tblItemArc WHERE isActive=1) itm ON itm.strItemName=l.strItemName AND itm.rn=1 GROUP BY l.strItemName`;
