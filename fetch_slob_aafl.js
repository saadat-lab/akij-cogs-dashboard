...
async function run(){
  try{
    ...
    periods[key2] = p2;
    await pool.close();
    const out = { sbu:'AAFL', sbuName:'AAFL Akij Agro Feed', source:'DataMart.inv.tblInventoryStatement', window:'6-mo trailing (as of '+asOf1+')', periods };
    fs.writeFileSync('C:\\Users\\saada\\OneDrive\\Documents\\Default Project\\slob_aafl.js','const SLOB_AAFL = '+JSON.stringify(out)+';');
    console.log('Saved slob_aafl.js (statement-based) as-of '+asOf1);
    Object.keys(periods).sort().reverse().forEach(k=>{
      const p=periods[k];
      console.log(k+' ('+(k===key1?asOf1:asOf2)+'): RM total='+p.values.RM.total.toFixed(2)+' SLOB='+p.values.RM.slob.toFixed(2)+' ('+p.values.RM.pct+'%) | PM total='+p.values.PM.total.toFixed(2)+' SLOB='+p.values.PM.slob.toFixed(2)+' ('+p.values.PM.pct+'%) | All='+p.values.totalInv.toFixed(2)+' SLOB='+p.values.slobTotal.toFixed(2));
    });
