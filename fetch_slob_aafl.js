...
async function run(){
  try{
    ...
    periods['2026-07'] = jul;
    await pool.close();
    const out = { sbu:'AAFL', sbuName:'AAFL Akij Agro Feed', source:'DataMart.inv.tblInventoryStatement', window:'6-mo trailing', periods };
    fs.writeFileSync('C:\\Users\\saada\\OneDrive\\Documents\\Default Project\\slob_aafl.js','const SLOB_AAFL = '+JSON.stringify(out)+';');
    console.log('Saved slob_aafl.js (statement-based)');
    ['2026-08','2026-07'].forEach(k=>{
      const p=periods[k];
      console.log(k+': RM total='+p.values.RM.total.toFixed(2)+' SLOB='+p.values.RM.slob.toFixed(2)+' ('+p.values.RM.pct+'%) | PM total='+p.values.PM.total.toFixed(2)+' SLOB='+p.values.PM.slob.toFixed(2)+' ('+p.values.PM.pct+'%) | All='+p.values.totalInv.toFixed(2)+' SLOB='+p.values.slobTotal.toFixed(2));
    });
    console.log('CSV says: Aug RM 95.64/0.01, PM 5.00/1.08, Total 100.64/1.09');
